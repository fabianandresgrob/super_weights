...
class MoESuperWeightDetector(BaseSuperWeightDetector):
    ...
    def _enhanced_routing_analysis(self, input_text: str, num_samples: int, 
        ...
        for sample_idx, sample_text in enumerate(sample_inputs):
            ...
            # Hook routers to capture detailed routing information
            routing_data = {}  # [layer] -> routing info per sample
    ...
    def _detect_layer_co_spikes(self, tokens, layer_idx: int, expert_indices: List[int],
        ...
        def create_routing_aware_hook(expert_idx: int, component_type: str):
            def hook(module, input, output):
                # Store activations - we'll filter by routing later
                ...
                # Store raw activations (will filter by routing)
                expert_activations[expert_idx] = {
                    'X': input_tensor,  # Pre-down-proj (expert input)
                    'Y': output_tensor,  # Post-down-proj (expert output) 
                    'component_type': component_type,
                    'module': module
                }
            return hook
        
        # Hook expert down-projection components
        hooks = []
                ...
                for comp_type, module in expert_components.items():
                    if comp_type in ['down', 'output']:
                        hook = module.register_forward_hook(
                            create_routing_aware_hook(expert_idx, comp_type)
                        )
                        hooks.append(hook)
                        break
            except Exception as e:
        ...
        # Process activations for each expert using only routed tokens
